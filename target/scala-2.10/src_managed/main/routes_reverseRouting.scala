...
def authenticate(): Call = {
   Call("POST", _prefix + { _defaultPrefix } + "signBack")
}
...
def authenticate : JavascriptReverseRoute = JavascriptReverseRoute(
   ...
   """
      function() {
      return _wA({method:"POST", url:"""" + _prefix + { _defaultPrefix } + """" + "signBack"})
      }
   """
)
...
def sign(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.Application.sign(), HandlerDef(this, "controllers.Application", "sign", Seq(), "GET", """Sign Up Page""", _prefix + """sign""")
)
                      

// @LINE:44
def authenticate(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.Application.authenticate(), HandlerDef(this, "controllers.Application", "authenticate", Seq(), "POST", """""", _prefix + """signBack""")
)
